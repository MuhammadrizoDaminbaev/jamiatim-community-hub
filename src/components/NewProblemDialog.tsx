...
export function NewProblemDialog({
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (cats.length === 0) return toast.error("Kamida bitta kategoriya tanlang");
    ...
    setFullname(""); setAge(""); setProblem(""); setSolution(""); setCats([]);
    onOpenChange(false);
    onCreated();
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="font-display text-3xl">Muammoyingizni yozing</DialogTitle>
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label>Kategoriyalar <span className="text-muted-foreground font-normal">(bir nechtasini tanlash mumkin)</span></Label>
              ...
              {CATEGORIES.map((c) => {
                ...
                return (
                  <button
                    ...
                    key={c.id}
                    onClick={() => toggle(c.id)}
                    className={`smooth px-3 py-1.5 rounded-full text-sm border ${
                      on ? "bg-foreground text-background border-foreground" : "bg-card hover:bg-accent"
                    }`}
                  >
                    {c.label}
                  </button>
          ...
          <div className="space-y-2">
            <Label>Yechim <span className="text-muted-foreground font-normal">(ixtiyoriy)</span></Label>
          ...
          <Button type="submit" className="w-full smooth font-medium" disabled={busy}>
